...
/**
    ...
    public void showFiles(int projectId){
        Map map;
        List<File> files = fileReferenceService.getFilesByProjectId(projectId);
...
/**
    ...
    public ResponseEntity<byte[]> downLoadFile(HttpServletRequest request, @RequestParam("fileName") String fileName){
        ...
        byte[] bytes;
        try {
            String path = request.getSession().getServletContext().getRealPath("upload") + java.io.File.separator;
